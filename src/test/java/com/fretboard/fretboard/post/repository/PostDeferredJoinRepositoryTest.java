...
class PostDeferredJoinRepositoryTest {
    ...
    @Test
    void 첫_페이지_최신순으로_size개_반환() {
        // given — createdAt 이 서로 다른 5건 저장
        ...
        assertThat(result)
                .as("첫 페이지는 created_at DESC 로 size 개를 반환해야 한다")
                .hasSize(3)
    ...
    void offset_적용_시_해당_구간_반환() {
        ...
        // when — offset=2, size=2 → 3,4번째 최신글
        List<PostSummaryDto> result = postRepository.findPostSummaryByBoardIdDeferred(board.getId(), 2, 2);

        // then
        assertThat(result)
                .as("offset 을 적용하면 해당 구간을 반환해야 한다")
                .hasSize(2)
                .extracting(PostSummaryDto::title)
    ...
    @Test
    void 동일_createdAt_경계에서_id_내림차순_타이브레이커로_중복_누락_없음() {
        // given — 동일한 createdAt 을 가진 4건 (id 만 서로 다름)
        ...
        // then — id DESC 타이브레이커로 중복/누락 없이 4건 전체가 순서대로 나와야 한다
